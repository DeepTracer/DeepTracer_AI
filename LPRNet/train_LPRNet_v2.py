...
def get_parser():
    ...
    parser.add_argument('--max_epoch', default=50, type=int, help='epoch to train the network')
    parser.add_argument('--img_size', default=[128, 32], nargs='+', type=int, help='the image size')
    ...
    parser.add_argument('--test_batch_size', default=60, type=int, help='testing batch size.')
    ...
    parser.add_argument('--save_folder', default='./weights_v4/', help='Location to save checkpoint models')
...
def train():
    ...
    train_dataset = LPRDataLoader(train_img_dirs.split(','), args.img_size, args.lpr_max_len, is_train=True)
    test_dataset = LPRDataLoader(test_img_dirs.split(','), args.img_size, args.lpr_max_len, is_train=False)
    ...
    for iteration in range(start_iter, max_iter):
        ...
        if iteration !=0 and iteration % args.save_interval == 0:
            torch.save(lprnet.state_dict(), args.save_folder + 'LPRNet_' + '_iteration_' + repr(iteration) + '_v2.pth')
        ...
        if loss.item() == np.inf or np.isnan(loss.item()):
            continue
        loss.backward()
        nn.utils.clip_grad_norm_(lprnet.parameters(), 5)
        optimizer.step()
        loss_val += loss.item()
        end_time = time.time()
        if iteration % 20 == 0:
    ...
    torch.save(lprnet.state_dict(), args.save_folder + 'Final_LPRNet_model_v2.pth')
...
def Greedy_Decode_Eval(Net, datasets, args):
    ...
    for batch_i in range(epoch_size):
        # load train data
        images, labels, lengths = next(batch_iterator)
        start = 0
        targets = []
        ...
        # [수정] 변수명 i -> sample_i 로 변경하여 충돌 방지
        for sample_i in range(prebs.shape[0]):
            preb = prebs[sample_i, :, :]
            preb_label = list()
        ...
        if batch_i == 0:
            for k in range(min(5, len(preb_labels))):
                ...
                my_predict = "".join([CHARS[idx] for idx in preb_labels[k]])
                print(f"   정답[{len(real_target)}]: {real_target}  vs  예측[{len(my_predict)}]: {my_predict}")

        for idx, label in enumerate(preb_labels):
            if len(label) != len(targets[idx]):
                Tn_1 += 1
                continue
            if (np.asarray(targets[idx]) == np.asarray(label)).all():
                Tp += 1
            else:
                Tn_2 += 1
